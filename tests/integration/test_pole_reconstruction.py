...
from zeroproof.training import HybridTeacher, create_pole_teacher
...
class TestPoleReconstruction:
    ...
    def test_pole_learning_with_supervision(self):
        ...
        if pole_head:
            teacher = create_pole_teacher(
                ...
                supervision_types=[
                    "proxy"
                ],  # Avoid pretraining; pole head params are TRNodes, not torch Tensors
        ...
        with torch.no_grad():
            q_abs_test = torch.tensor(model.get_q_values(x_test), dtype=torch.float32).reshape(
                -1, 1
            )
        ...
        assert (
            accuracy >= config.min_pole_accuracy
        ...
        ple, _ = compute_pole_localization_error(clustered_poles, config.true_poles)
        print(f"  PLE: {ple:.4f}")
        assert ple <= config.max_ple, f"PLE {ple:.4f} > threshold {config.max_ple}"
    ...
    def test_pole_metrics(self):
        ...
        model = create_enhanced_rational(
            d_p=config.degree_p, d_q=4, basis_type="polynomial", enable_pole_detection=True
        )
        ...
        for xv in enforce_xs:
            ...
            if denom > 0 and len(model.phi) >= len(row):
                ...
                for k, v in enumerate(row[:-1]):
                    s += (
                        float(model.phi[k].value.value) * v
                        if model.phi[k].value.tag == TRTag.REAL
                        else 0.0
                    )
                ...
                if abs(abs(v_last) - 1.0) < 1e-7:
                    ...
                    s_full = s + (
                        float(last.value.value) * v_last if last.value.tag == TRTag.REAL else 0.0
                    )
        ...
        assert (
            sign_consistency >= config.min_sign_consistency
        ...
        slope_error, _ = compute_asymptotic_slope_error(
            x_test.numpy(), y_pred.numpy() if hasattr(y_pred, "numpy") else y_pred, q_true.numpy()
        )
        print(f"  Asymptotic slope error: {slope_error:.4f}")
        assert (
            slope_error <= config.max_slope_error
        ...
        assert (
            float(res_mean) <= config.max_residual
