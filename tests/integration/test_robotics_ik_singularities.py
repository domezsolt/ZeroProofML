...
from zeroproof.layers import EnhancedTRRational, TRRational
...
class TwoLinkRobot:
    ...
    def generate_workspace_samples(
        self, n_samples: int, include_singular: bool = True
...
class TestRoboticsIKSingularities:
    ...
    def test_ik_with_singularities(self):
        ...
        for epoch in range(config.n_epochs):
            ...
            singular_labels = teacher.get_pole_labels(
                q_pred, robot_params={"l1": robot.l1, "l2": robot.l2}
            )
            ...
            if q_pred.grad is not None:
                ...
                if len(grad_near_singular) > 0:
                    ...
                    assert max_grad < 1000.0, f"Gradient explosion near singularity: {max_grad:.2e}"
        ...
        assert singular_encounters >= 1, f"Too few singularities: {singular_encounters} < 1"
        ...
        assert (
            abs(final_coverage - config.target_coverage) < 0.13
    ...
    def test_singularity_aware_sampling(self):
        ...
        if len(batch_indices) > 0:
            ...
            assert (
                near_singular_ratio > pool_near_singular
