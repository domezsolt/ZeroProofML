...
from zeroproof.utils.parallel import (
    ParallelConfig, TRThreadPool, TRProcessPool, parallel_map, parallel_reduce,
    vectorize_operation, ParallelTRComputation, batch_tr_operation,
    parallel_graph_eval, ParallelTRArray
)
...
class TestVectorization:
    ...
    def test_vectorize_numpy_operation(self):
        ...
        inputs = np.array([[real(1.0), real(2.0)],
                          [real(3.0), real(4.0)]])
...
class TestParallelComputation:
    ...
    def test_dependency_execution(self):
        ...
        comp.add_task('c', lambda: task_c(comp._results['a'], comp._results['b']),
                     depends_on=['a', 'b'])
