...
from zeroproof.layers.tag_aware_rational import (
    TagAwareRational,
...
class TestTagPredictionHead:
    ...
    def test_initialization(self):
        ...
        expected_params = (
            (3 + 1) * 8 +  # W1: (input_dim+1) x hidden_dim
            8 +             # b1: hidden_dim
            8 * 4 +         # W2: hidden_dim x 4
            4               # b2: 4
        )
    ...
    def test_predict_tag(self):
        ...
        assert pred_class in [TagClass.REAL, TagClass.PINF, 
                             TagClass.NINF, TagClass.PHI]
...
class TestTagLossIntegration:
    ...
    def test_compute_tag_loss_with_mixed_tags(self):
        ...
        # Create mock logits
        tag_logits = [
            [TRNode.constant(real(1.0)), TRNode.constant(real(0.0)),
             TRNode.constant(real(0.0)), TRNode.constant(real(0.0))],  # Predict REAL
            [TRNode.constant(real(0.0)), TRNode.constant(real(1.0)),
             TRNode.constant(real(0.0)), TRNode.constant(real(0.0))],  # Predict PINF
            [TRNode.constant(real(0.0)), TRNode.constant(real(0.0)),
             TRNode.constant(real(1.0)), TRNode.constant(real(0.0))],  # Predict NINF
            [TRNode.constant(real(0.0)), TRNode.constant(real(0.0)),
             TRNode.constant(real(0.0)), TRNode.constant(real(1.0))]   # Predict PHI
        ]
