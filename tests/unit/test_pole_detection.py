...
from zeroproof.layers.pole_aware_rational import (
    PoleAwareRational,
...
class TestPoleLoss:
    ...
    def test_compute_pole_loss_with_teacher(self):
        ...
        loss = compute_pole_loss(
            predictions, pole_scores, None, teacher_labels, config
        )
    ...
    def test_compute_pole_loss_with_Q_values(self):
        ...
        loss = compute_pole_loss(
            predictions, pole_scores, Q_values, None, config
        )
    ...
    def test_compute_pole_loss_with_tags(self):
        ...
        loss = compute_pole_loss(
            predictions, pole_scores, None, None, None
        )
...
class TestPoleAwareRational:
    ...
    def test_initialization(self):
        ...
        basis = MonomialBasis()
        layer = PoleAwareRational(
            d_p=2, d_q=2, basis=basis,
...
class TestDomainSpecificDetector:
    ...
    def test_robotics_singularity(self):
        ...
        is_singular = detector.get_robotics_singularity(
            [0.0, 0.0], robot_type="RR"
        )
        assert is_singular == True  # θ2=0 is singular
        
        is_not_singular = detector.get_robotics_singularity(
            [0.0, np.pi/2], robot_type="RR"
        )
