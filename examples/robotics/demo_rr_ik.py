...
def demonstrate_rr_kinematics():
    ...
    for theta1, theta2 in test_configs:
        ...
        print(f"{theta1:.3f}\t\t{theta2:.3f}\t\t{x_ee:.3f}\t\t{y_ee:.3f}\t\t{det_J:.6f}\t\t{is_singular}")
...
def demonstrate_dataset_generation():
    ...
    for i, sample in enumerate(samples[:5]):
        print(f"{sample.theta1:.3f}\t\t{sample.theta2:.3f}\t\t{sample.dx:.3f}\t\t{sample.dy:.3f}\t\t"
...
def demonstrate_training_comparison():
    ...
    for name, config in configs:
        ...
        results[name] = {
            ...
            'n_parameters': len(trainer.model.parameters()) if hasattr(trainer.model, 'parameters') else 0
        }
    ...
    for name, result in results.items():
        print(f"{name:<15}\t{result['mse']:.6f}\t{result['training_time']:.2f}\t\t{result['n_parameters']}")
...
def demonstrate_singularity_analysis():
    ...
    for theta2 in theta2_values:
        ...
        print(f"{theta2:.3f}\t\t{x_ee:.3f}\t\t{y_ee:.3f}\t\t{det_J:.6f}\t\t{manip:.6f}\t\t{is_singular}{marker}")
