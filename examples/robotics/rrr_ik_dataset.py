...
class RRRKinematics:
    ...
    def forward_kinematics(self, theta1: float, theta2: float, theta3: float) -> Tuple[float, float]:
        t12 = theta1 + theta2
        t123 = t12 + theta3
    ...
    def is_singular(self, theta1: float, theta2: float, theta3: float, threshold: float = 1e-3) -> bool:
        return bool(self.manipulability_index(theta1, theta2, theta3) < threshold)

    def dls_ik(self, theta1: float, theta2: float, theta3: float,
               dx: float, dy: float, damping: float = 0.01) -> Tuple[float, float, float]:
...
class RRRDatasetGenerator:
    ...
    def sample_configurations(self,
                              ...
                              min_manip_regular: Optional[float] = None) -> List[Tuple[float, float, float]]:
    ...
    def generate_ik_samples(self,
                            ...
                            damping_factor: float = 0.01) -> List[IK3RSample]:
        ...
        for t1, t2, t3 in configurations:
            ...
            samples.append(IK3RSample(
                dx=dx, dy=dy,
                theta1=float(t1), theta2=float(t2), theta3=float(t3),
                dtheta1=float(d1), dtheta2=float(d2), dtheta3=float(d3),
                det_J=float(manip),  # compatibility: use manip as det_J proxy
                cond_J=float(cond),
                is_singular=bool(is_sing),
                x_ee=float(x_ee), y_ee=float(y_ee),
    ...
    def generate_dataset(self,
                         ...
                         min_manip_regular: Optional[float] = None) -> List[IK3RSample]:
        ...
        print(f"Generated samples: {len(self.samples)}  singular: {n_sing} ({n_sing/len(self.samples):.1%})")
        return self.samples

    @staticmethod
    def stratify_split(samples: List[IK3RSample],
                       train_ratio: float = 0.8,
                       edges: List[float] = None) -> Dict[str, List[IK3RSample]]:
    ...
    def save_dataset(self, filename: str, format: str = "json") -> None:
        ...
        if format == 'json':
            ...
            arrays = {}
            keys = ['dx','dy','theta1','theta2','theta3','dtheta1','dtheta2','dtheta3',
                    'det_J','cond_J','x_ee','y_ee','manipulability','distance_to_singularity']
            for key in keys:
...
def main():
    ...
    ap.add_argument('--bucket_edges', type=str, default=None, help='Comma-separated, last can be inf')
    ...
    if args.stratify_by_manip:
        split = RRRDatasetGenerator.stratify_split(samples, train_ratio=args.train_ratio, edges=bucket_edges)
        split_info = {
            'train_bucket_counts': {},
            'test_bucket_counts': {},
        }
        ...
        if args.ensure_buckets_nonzero:
            def augment(subset: List[IK3RSample], counts: List[int]) -> List[IK3RSample]:
                ...
                while missing and attempts < 400:
                    ...
                    new_s = generator.generate_ik_samples([(t1, t2, t3)], args.displacement_scale, args.damping_factor)[0]
