...
def visualize_coverage_control(trainer: HybridTRTrainer, 
                               test_x: List,
                               test_y: List):
    """
    ...
    if 'coverage' in history:
        ...
        if trainer.coverage_policy:
            ...
            ax.axhline(y=target, color='blue', linestyle='--', 
                      label=f'Target ({target:.2f})', alpha=0.7)
            ax.axhline(y=min_cov, color='red', linestyle='--', 
                      label=f'Min ({min_cov:.2f})', alpha=0.7)
    ...
    if 'lambda_rej' in history and len(history['lambda_rej']) > 0:
        ...
        if trainer.coverage_policy:
            ...
            for interv in interventions:
                ax.axvline(x=interv['epoch'], color='orange', 
                          linestyle=':', alpha=0.7)
    ...
    scatter = ax.scatter(x_vals, y_pred_vals, c=tag_colors, 
                        alpha=0.6, s=20)
    ax.set_xlabel('x')
    ax.set_ylabel('y')
...
def run_coverage_control_comparison():
    ...
    trainer_no_control = HybridTRTrainer(
        model=model_no_control,
        config=config_no_control
    )
    ...
    model_lagrange = HybridTRRational(
        d_p=3,
        d_q=3,
        basis=MonomialBasis(),
        alpha_phi=1e-3
    ...
    trainer_lagrange = HybridTRTrainer(
        model=model_lagrange,
        config=config_lagrange
    )
    ...
    trainer_pid = HybridTRTrainer(
        model=model_pid,
        config=config_pid
    )
    
    history_pid = trainer_pid.train(
        train_data=[(train_x, train_y)],
        val_data=[(test_x, test_y)]
    )
    ...
    print("-" * 50)
    print(f"{'No Control':<20} {metrics_no_control['loss']:<12.6f} "
          f"{metrics_no_control['coverage']:<12.3f} {metrics_no_control['lambda']:<12.3f}")
    print(f"{'Lagrange Control':<20} {metrics_lagrange['loss']:<12.6f} "
          f"{metrics_lagrange['coverage']:<12.3f} {metrics_lagrange['lambda']:<12.3f}")
    print(f"{'PID Control':<20} {metrics_pid['loss']:<12.6f} "
