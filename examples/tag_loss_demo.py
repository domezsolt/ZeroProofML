...
def visualize_tag_predictions(model: TagAwareRational, 
                              test_x: List,
                              test_y: List):
    """
    ...
    for i in range(0, len(x_grid) - window_size, window_size // 2):
        ...
        correct = sum(1 for t, p in zip(window_tags_true, window_tags_pred)
                     if TagClass.from_tag(t) == p)
...
def run_tag_loss_demo():
    ...
    model_notag = TagAwareRational(
        ...
        basis=MonomialBasis(),
        alpha_phi=1e-3,
        enable_tag_head=False  # No tag head
    )
    ...
    trainer_withtag = HybridTRTrainer(
        model=model_withtag,
        config=config_withtag
    )
