...
class RationalEpsTrainer:
                ...
                try:
                    ...
                    for param in self.model.parameters():
                        if param.gradient and param.gradient.tag == TRTag.REAL:
                            if (np.isnan(param.gradient.value) or 
                                np.isinf(param.gradient.value)):
                                gradient_ok = False
                                break
    ...
    def train(self, 
        ...
        print(f"Training Rational+ε (ε={self.model.epsilon}) for {self.model.config.epochs} epochs...")
...
def grid_search_epsilon(train_data: Tuple[List, List],
    ...
    for epsilon in config.epsilon_values:
        ...
        print(f"ε={epsilon}: MSE={val_mse:.6f}, "
    ...
    with open(csv_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Epsilon', 'Val_MSE', 'Training_Time', 'NaN_Count', 'Inf_Count', 'Success'])
...
def run_rational_eps_baseline(train_data: Tuple[List, List],
...
if __name__ == "__main__":
    ...
    parser.add_argument('--degree_p', type=int, default=3,
                       help='Numerator polynomial degree')
    parser.add_argument('--degree_q', type=int, default=2,
                       help='Denominator polynomial degree')
    ...
    parser.add_argument('--output_dir', default='results/rational_eps_baseline',
