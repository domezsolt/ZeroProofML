% ZeroProofML — Full Article (Theory + Empirics)
...
\label{sec:empirical}
We validate the theory on a suite of IK tasks with explicit singular structure.
Across seeds (2R), TR--Full matches or exceeds the strongest $\varepsilon$/smooth baselines in overall error and attains \emph{1.5--2.5$\times$ lower error in the tightest near--pole bins (B0--B1)}, with \(\sim\)3--4\% gains in B2 and near parity in B3 (Table~\ref{tab:near_pole_bins}; see also Table~\ref{tab:overall_mse}). Under closed--loop tracking, TR--Full yields the lowest task--space error and smallest joint steps (Table~\ref{tab:rollout}).

These observations align with the bounded--update guarantee (Prop.~\ref{prop:bounded-update}) and the batch--safe step rules in Sec.~\ref{sec:global-convergence}: gradients remain bounded due to Mask--REAL on REAL paths and saturated surrogates otherwise; the controller enforces near--pole coverage without destabilizing steps. The deterministic policy (ULP--scaled guard bands, hysteresis, deterministic reductions) leads to essentially zero across--seed variance on TR bins, consistent with Sec.~\ref{sec:policy-determinism} and the guard--band analysis in Sec.~\ref{sec:precision-tags}.

Beyond 2R, we probe multi--singularity structure on planar 3R: TR--Full exhibits finite pole--localization error (PLE), stable sign behavior across \(\theta_2,\theta_3\) singular lines, and small forward--kinematics residuals (Table~\ref{tab:e3r}), supporting robustness near rank--deficient Jacobians under the hybrid policy. Synthetic 6R results (Table~\ref{tab:ik6r}) further corroborate stability and accuracy in higher--DOF settings.

In sum, the empirical outcomes mirror the theoretical picture: totalization and tag--aware autodiff prevent undefined behaviors; hybrid switching and coverage control prioritize learning near poles without compromising stability; policy determinism yields reproducible outcomes up to declared ULP bands.

\paragraph{Tables and Figures.} For completeness, we include the tables and figure summarizing the empirical results used in this section.
\begin{itemize}
  \item Overall (across seeds): \input{results/robotics/paper_suite/latex/overall_table.tex}
  \item Near--pole bins B0--B2 (across seeds): \input{results/robotics/paper_suite/latex/near_pole_table.tex}
  \item Closed--loop tracking summary: \input{results/robotics/paper_suite/latex/rollout_table.tex}
  \item 3R near--pole metrics (TR): \input{results/robotics/paper_suite/latex/e3r_table.tex}
  \item 6R synthetic summary (TR): \input{results/robotics/paper_suite/latex/ik6r_table.tex}
\end{itemize}
Optionally, a compact bar plot for B0--B2 is shown in Fig.~\ref{fig:b012bars-theory}.
\begin{figure}[h]
  \centering
