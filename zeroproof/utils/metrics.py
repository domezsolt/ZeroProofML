...
from ..core import TRScalar, TRTag, real, tr_add, tr_mul, tr_div, tr_sub
...
class PoleLocalizationError:
    ...
    def find_poles_1d(self, 
                     model, 
                     x_range: Tuple[float, float],
                     n_samples: int = 1000,
                     q_threshold: float = 5e-2) -> List[PoleLocation]:
        """
        ...
        for i in range(1, len(q_values) - 1):
            if (q_values[i] < adaptive_threshold and 
                q_values[i] < q_values[i-1] and 
    ...
    def _refine_pole_location(self, 
                             x_vals: List[float], 
                             q_vals: List[float]) -> float:
        """Refine pole location using parabolic interpolation."""
    ...
    def compute_chamfer_distance(self, 
                                predicted: List[PoleLocation],
                                ground_truth: List[PoleLocation]) -> float:
        """
        ...
        for pred_pole in predicted:
            pred_to_gt_sum += min(
                pred_pole.distance_to(gt_pole) for gt_pole in ground_truth
            )
        gt_to_pred_sum = 0.0
        for gt_pole in ground_truth:
            gt_to_pred_sum += min(
                gt_pole.distance_to(pred_pole) for pred_pole in predicted
            )
    ...
    def compute_hausdorff_distance(self, 
        ...
        pred_to_gt = max(min(pred_pole.distance_to(gt_pole) 
                            for gt_pole in ground_truth)
                        for pred_pole in predicted)
        
        # Max distance from ground truth to predicted
        gt_to_pred = max(min(gt_pole.distance_to(pred_pole) 
                            for pred_pole in predicted)
                        for gt_pole in ground_truth)
        
        return max(pred_to_gt, gt_to_pred)
    
    def compute_ple(self, 
                   model,
...
class AsymptoticSlopeAnalyzer:
    ...
    def compute_asymptotic_slope(self,
                                model,
                                pole_location: float,
        ...
        for x_val in np.concatenate([x_left, x_right]):
            try:
                ...
                if (tag == TRTag.REAL and 
                    abs(y.value.value) > 1e-12 and 
                    q_abs is not None and 
                    q_abs > 1e-12):
            ...
            try:
                ...
                if abs(denom) > 1e-12:
                    ...
                    ss_res = sum((y - (slope * x + intercept))**2 
                                for x, y in zip(log_q_vals, log_y_vals))
...
class ResidualConsistencyLoss:
    ...
    def compute_residual(self, 
                        model, 
                        x: Union[float, TRNode]) -> Optional[TRNode]:
        """
    ...
    def compute_loss(self, 
                    model, 
                    inputs: List[Union[float, TRNode]],
                    near_pole_threshold: float = 0.2) -> TRNode:
        """
...
class AntiIllusionMetrics:
    ...
    def evaluate_model(self,
                      ...
                      x_range: Tuple[float, float] = (-2, 2)) -> Dict[str, float]:
        """
        ...
        for pole in ground_truth_poles:
            slope_metrics = self.slope_analyzer.compute_asymptotic_slope(
                model, pole.x
            )
    ...
    def get_trends(self, window_size: int = 10) -> Dict[str, str]:
        ...
        for metric in ['ple', 'sign_consistency', 'asymptotic_slope_error', 
                      'residual_consistency', 'anti_illusion_score']:
