...
class PoleVisualizer:
    ...
    def plot_Q_comparison(self,
        ...
        # Plot Q curves
        ax.plot(x_values, Q_learned, 
                color=self.colors['Q_curve'], 
                linewidth=2, 
                label='Learned Q(x)')
        
        if Q_true is not None:
            ax.plot(x_values, Q_true, 
                   color='gray', 
                   linewidth=2, 
                   linestyle='--',
                   label='True Q(x)')
    ...
    def plot_gradient_flow(self,
        ...
        if true_poles:
            for pole in true_poles:
                ...
                rect = Rectangle((pole - 0.1, 0), 0.2, max(gradients),
    ...
    def plot_coverage_breakdown(self,
                               x_values: List[float],
                               y_tags: List[TRTag],
                               true_poles: List[float],
        ...
        for pole in true_poles:
            # Near region
            rect_near = Rectangle((pole - near_threshold, -0.1), 
                                 2 * near_threshold, 1.2,
                                 color=self.colors['near_pole'], 
                                 alpha=0.2,
            ...
            # Mid region
            rect_mid_left = Rectangle((pole - mid_threshold, -0.1), 
                                     mid_threshold - near_threshold, 1.2,
                                     color=self.colors['mid_range'], 
                                     alpha=0.2,
                                     label='Mid-range' if pole == true_poles[0] else None)
            ax.add_patch(rect_mid_left)
            
            rect_mid_right = Rectangle((pole + near_threshold, -0.1), 
                                      mid_threshold - near_threshold, 1.2,
                                      color=self.colors['mid_range'], 
                                      alpha=0.2)
        ...
        # Mark poles
        for pole in true_poles:
            ax.axvline(x=pole, color=self.colors['true_pole'], 
                      linestyle='--', alpha=0.5,
    ...
    def plot_asymptotic_behavior(self,
                                x_values: List[float],
                                y_values: List[float],
                                Q_values: List[float],
        ...
        for i, (y, q) in enumerate(zip(y_values, Q_values)):
            if abs(y) > 1e-10 and abs(q) > 1e-10:
                valid_points.append((np.log(abs(y)), -np.log(abs(q)), 
        ...
        ax1.plot([min_val, max_val], [min_val, max_val], 
                'k--', alpha=0.5, label='Ideal (slope=1)')
        ...
        if len(near_indices) > 1:
            ...
            p = np.poly1d(z)
            ax1.plot(sorted(near_x), p(sorted(near_x)), 
                    'r-', alpha=0.7, 
