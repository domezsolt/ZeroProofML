...
class TROptimizer:
    ...
    def _constant_fold(self, node: TRNode) -> TRNode:
        ...
        all_constant = all(
            not inp.requires_grad and not inp._grad_info 
            for inp in inputs
        )
...
class GraphOptimizer:
    ...
    def _setup_default_rules(self):
        ...
        # Rule: x + 0 -> x
        self.add_rule(
            lambda n: (n._grad_info and n._grad_info.op_type == OpType.ADD and
                      any(self._is_zero(inp()) for inp in n._grad_info.inputs if inp())),
            lambda n: self._apply_add_zero_rule(n)
        )
        
        # Rule: x * 1 -> x
        self.add_rule(
            lambda n: (n._grad_info and n._grad_info.op_type == OpType.MUL and
                      any(self._is_one(inp()) for inp in n._grad_info.inputs if inp())),
    ...
    def add_rule(self, pattern: Callable[[TRNode], bool], 
                 rewrite: Callable[[TRNode], TRNode]):
    ...
    def _is_zero(self, node: Optional[TRNode]) -> bool:
        ...
        if node is None:
            return False
        return (node.value.tag == TRTag.REAL and 
                node.value.value == 0.0 and
                not node._grad_info)
...
class OperationFuser:
    ...
    def _detect_linear_pattern(self, nodes: List[TRNode]) -> bool:
        ...
        return (nodes[0]._grad_info and 
                nodes[0]._grad_info.op_type == OpType.MUL and
                nodes[1]._grad_info and 
                nodes[1]._grad_info.op_type == OpType.ADD)
