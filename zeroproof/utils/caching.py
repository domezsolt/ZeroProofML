    ...
    def put(self, key: str, value: Any, size: Optional[int] = None, 
        ...
        with self._lock:
            ...
            while (len(self._cache) >= self.max_size or 
                   self._memory_used + size > self.max_memory_bytes):
...
def memoize_tr(cache: Optional[TRCache] = None,
...
def cached_operation(op_type: str, 
                    inputs: Tuple[TRScalar, ...]) -> Optional[TRScalar]:
    """
...
def cache_operation_result(op_type: str,
                          inputs: Tuple[TRScalar, ...],
                          result: TRScalar):
    """
...
class ResultCache:
    ...
    def get_or_compute(self, 
                       node: TRNode,
                       compute_func: Callable[[TRNode], Any]) -> Any:
        """
    ...
    def _cache_result(self, node: TRNode, result: Any):
        ...
        if len(self._cache) >= self.max_entries:
            ...
            evict_id = min(self._cache.keys(),
                          key=lambda k: len(self._dependents.get(k, set())))
    ...
    def _node_unchanged(self, node1: TRNode, node2: TRNode) -> bool:
        ...
        return (node1.value == node2.value and
                node1.requires_grad == node2.requires_grad)
