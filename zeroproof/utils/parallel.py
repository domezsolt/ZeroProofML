...
def parallel_reduce(func: Callable,
    ...
    chunks = [items_list[i:i + chunk_size] 
        ...
        with TRThreadPool(self.config.num_workers) as pool:
            ...
            for task in sorted_tasks:
                ...
                future.add_done_callback(
                    functools.partial(store_result, task['id'])
                )
...
def batch_tr_operation(operation: Callable,
                      inputs: List[Tuple[TRScalar, ...]],
                      batch_size: int = 1000,
                      parallel: bool = True) -> List[TRScalar]:
    """
...
def parallel_graph_eval(root_nodes: List[TRNode],
                       config: Optional[ParallelConfig] = None) -> List[Any]:
    """
...
class ParallelTRArray:
    ...
    @staticmethod
    def add(a: List[TRScalar], b: List[TRScalar],
            parallel: bool = True) -> List[TRScalar]:
    ...
    @staticmethod
    def mul(a: List[TRScalar], b: List[TRScalar],
            parallel: bool = True) -> List[TRScalar]:
        ...
        from ..autodiff import tr_mul
        return parallel_map(
            lambda pair: tr_mul(pair[0], pair[1]),
            list(zip(a, b)),
            ParallelConfig(backend='thread')
        )
    
    @staticmethod
    def reduce_sum(values: List[TRScalar],
                   parallel: bool = True) -> TRScalar:
