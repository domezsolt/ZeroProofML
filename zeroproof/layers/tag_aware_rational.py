...
from ..training.tag_loss import TagPredictionHead, TagClass
...
class TagAwareRational(HybridTRRational):
    ...
    def __init__(self,
        ...
        if enable_tag_head:
            ...
            self.tag_head = TagPredictionHead(
                input_dim=d_q,
                hidden_dim=tag_head_hidden_dim,
                basis=basis
            )
        else:
            self.tag_head = None
    ...
    def forward_with_tag_pred(self, 
    ...
    def get_tag_statistics(self) -> dict:
        ...
        # Compute accuracy
        correct = sum(1 for h in self.tag_prediction_history 
                     if h['true'] == h['pred'])
        ...
        for cls in TagClass:
            class_preds = [h for h in self.tag_prediction_history 
                          if h['true'] == cls]
            if class_preds:
                class_correct = sum(1 for h in class_preds 
                                  if h['pred'] == cls)
...
class TagAwareMultiRational:
    ...
    def forward_with_tag_pred(self, 
                              x: Union[TRScalar, TRNode]
                              ) -> Tuple[List[Tuple[TRNode, TRTag]], 
                                       Optional[List[TRNode]]]:
        """
