...
class EnhancedPoleDetectionHead:
    ...
    def forward(self, x: Union[TRNode, TRScalar], 
                features: Optional[List[TRNode]] = None) -> TRNode:
        """
    ...
    def compute_loss(self,
                     ...
                     teacher_labels: Optional[List[float]] = None) -> TRNode:
        """
        ...
        for i, pred in enumerate(predictions):
            ...
            if teacher_labels and i < len(teacher_labels):
                ...
                if self.config.label_smoothing > 0:
                    teacher_target = (1 - self.config.label_smoothing) * teacher_target + \
                ...
                neg_term = tr_mul(
                    ...
                    tr_neg(self._safe_log(tr_add(
                        tr_sub(TRNode.constant(real(1.0)), prob),
                        epsilon
                    )))
                )
                bce_loss = tr_mul(
                    TRNode.constant(real(weight)),
                    tr_add(pos_term, neg_term)
                )
                
                loss_components.append(tr_mul(
                    TRNode.constant(real(self.config.teacher_weight)),
                    bce_loss
                ))
            ...
            if Q_values and i < len(Q_values):
                ...
                loss_components.append(tr_mul(
                    TRNode.constant(real(self.config.self_supervised_weight)),
                    self_loss
                ))
        ...
        if losses:
            ...
            weighted_loss = tr_mul(
                TRNode.constant(real(self.config.pole_loss_weight)),
                avg_loss
            )
    ...
    def regularization_loss(self) -> TRNode:
        ...
        reg_loss = tr_mul(
            TRNode.constant(real(self.config.weight_decay * 0.5)),
            reg_loss
        )
    ...
    def compute_regularization(self, 
        ...
        for pole_loc in self.target_poles:
            ...
            for offset in [-0.1, -0.05, 0.0, 0.05, 0.1]:
                ...
                closest_idx = min(range(len(x_samples)), 
                                key=lambda i: abs(x_samples[i] - x_near))
        ...
        if self.target_poles:
            ...
            reg_loss = tr_mul(
                TRNode.constant(real(self.regularization_weight)),
                reg_loss
            )
...
class PoleAwareRationalInterface:
    ...
    def compute_pole_loss(self,
        ...
        """
        # Pole detection loss
        detection_loss = self.pole_head.compute_loss(
            predictions, Q_values, teacher_labels
        )
