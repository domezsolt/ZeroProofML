...
from ..autodiff import tr_add, tr_mul, tr_div, tr_sub, tr_pow_int, tr_abs
...
def softmax(logits: List[TRNode], temperature: float = 1.0) -> List[TRNode]:
    ...
    logits = [logit if isinstance(logit, TRNode) else TRNode.constant(real(float(logit))) for logit in logits]
...
def cross_entropy_loss(pred_probs: List[TRNode], 
                       true_class: TagClass,
                       epsilon: float = 1e-7) -> TRNode:
    """
...
def compute_tag_accuracy(predictions: List[TRNode],
                         tag_predictions: List[TagClass]) -> float:
    """
...
def compute_tag_confusion_matrix(predictions: List[TRNode],
                                tag_predictions: List[TagClass]) -> Dict[str, Dict[str, int]]:
    """
