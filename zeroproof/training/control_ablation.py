...
class AblationRunner:
    ...
    def get_best_strategy(self) -> Tuple[str, Dict[str, Any]]:
        ...
        for name, result in self.results.items():
            ...
            score = (
                0.3 * m['settling_time'] / self.config.n_epochs +  # Normalized settling time
                0.2 * m['overshoot'] +  # Overshoot penalty
                0.3 * m['steady_state_error'] +  # Steady-state accuracy
                0.1 * m['oscillations'] / self.config.n_epochs +  # Normalized oscillations
                0.1 * m['control_effort'] / 100  # Normalized effort
            )
            scores[name] = score
...
def run_control_ablation(strategies: Optional[List[str]] = None,
                        target_coverage: float = 0.85,
                        n_epochs: int = 100,
