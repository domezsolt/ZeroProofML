...
from zeroproof.utils import (
    TRBenchmark, OperationBenchmark, create_scaling_benchmark,
...
class ComprehensiveBenchmarks:
    ...
    def run_autodiff_benchmarks(self):
        ...
        results['chain_rule'] = benchmark.benchmark(
            chain_rule,
            name='chain_rule',
            iterations=500
        )
    ...
    def run_layer_benchmarks(self):
        ...
        def norm_forward():
            batch = [[TRNode.constant(zp.real(float(i+j))) 
                     for j in range(10)] 
                     for i in range(32)]
    ...
    def run_parallel_benchmarks(self):
        ...
        for num_workers in [2, 4, 8]:
            ...
            results[f'parallel_{num_workers}'] = benchmark.benchmark(
                parallel,
                name=f'parallel_{num_workers}_workers',
                iterations=1,
                samples=5
            )
...
def main():
    ...
    parser.add_argument('--suite', nargs='+',
                       choices=['arithmetic', 'autodiff', 'layers', 'scaling', 
                               'parallel', 'memory', 'caching', 'all'],
                       default=['all'],
