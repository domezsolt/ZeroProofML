...
def create_professional_schematic():
    ...
    ax.text(2.5, 8.7, '$x \\in \\mathbb{R}^n$ (joint angles)', ha='center', va='center', fontsize=10)
    
    # Layer 2: TR-Rational Layer
    tr_box = FancyBboxPatch((6, 8), 4, 2, boxstyle="round,pad=0.1",
                           facecolor=colors['tr_layer'], edgecolor='#388E3C', linewidth=2)
    ax.add_patch(tr_box)
    ...
    decision_diamond = plt.Polygon(decision_points, facecolor=colors['decision'], 
    ...
    # Guard Path (Left)
    guard_box = FancyBboxPatch((1, 2.5), 4, 2, boxstyle="round,pad=0.1",
                              facecolor=colors['guard'], edgecolor='#F57C00', linewidth=2)
    ...
    ax.text(3, 2.7, 'Tag: REAL', ha='center', va='center', fontsize=10, style='italic', color='darkgreen')
    
    # Real Path (Right)
    real_box = FancyBboxPatch((9, 2.5), 4, 2, boxstyle="round,pad=0.1",
                             facecolor=colors['real'], edgecolor='#D32F2F', linewidth=2)
    ...
    ax.text(11, 2.7, 'Tag: INF/NULL', ha='center', va='center', fontsize=10, style='italic', color='darkred')
    
    # Coverage Controller (Top Right)
    controller_box = FancyBboxPatch((11, 7), 3, 2, boxstyle="round,pad=0.1",
                                   facecolor=colors['control'], edgecolor='#689F38', linewidth=2)
    ...
    # Output Layer
    output_box = FancyBboxPatch((6, 0.5), 4, 1.5, boxstyle="round,pad=0.1",
    ...
    ax.text(8, 0.8, 'Tagged transreal values', ha='center', va='center', fontsize=10, style='italic')
    ...
    ax.annotate('', xy=(4.5, 4.5), xytext=(6.8, 5.2), 
                arrowprops=dict(arrowstyle='->', lw=2.5, color='green'))
    ax.text(5.5, 5.2, 'YES', fontsize=11, color='green', weight='bold', 
            bbox=dict(boxstyle="round,pad=0.2", facecolor='white', edgecolor='green'))
    
    # Decision to Real (NO)
    ax.annotate('', xy=(9.5, 4.5), xytext=(9.2, 5.2),
                arrowprops=dict(arrowstyle='->', lw=2.5, color='red'))
    ax.text(9.8, 5.2, 'NO', fontsize=11, color='red', weight='bold',
            bbox=dict(boxstyle="round,pad=0.2", facecolor='white', edgecolor='red'))
    
    # Both paths to output
    ax.annotate('', xy=(7, 2), xytext=(3, 2.5), 
                arrowprops=dict(arrowstyle='->', lw=2.5, color='orange'))
    ax.annotate('', xy=(9, 2), xytext=(11, 2.5),
                arrowprops=dict(arrowstyle='->', lw=2.5, color='red'))
    
    # Coverage controller feedback (dashed)
    ax.annotate('', xy=(10, 8), xytext=(11, 7.5),
                arrowprops=dict(arrowstyle='->', lw=2, color='brown', linestyle='--'))
    ax.text(10.2, 7.2, 'Adaptive\\nFeedback', fontsize=9, color='brown', weight='bold',
            ha='center', va='center')
    
    # Add mathematical details in boxes
    math_box1 = FancyBboxPatch((0.5, 5.5), 3.5, 1.5, boxstyle="round,pad=0.1",
                              facecolor='#F5F5F5', edgecolor='gray', linewidth=1)
    ax.add_patch(math_box1)
    ax.text(2.25, 6.6, 'Mathematical Foundation', ha='center', va='center', fontsize=10, weight='bold')
    ax.text(2.25, 6.2, '$\\mathbb{T} = \\mathbb{R} \\cup \\{+\\infty, -\\infty, \\Phi\\}$', ha='center', va='center', fontsize=10)
    ax.text(2.25, 5.8, 'Total arithmetic', ha='center', va='center', fontsize=9, style='italic')
    
    math_box2 = FancyBboxPatch((10.5, 0.5), 3.5, 1.5, boxstyle="round,pad=0.1",
                              facecolor='#F5F5F5', edgecolor='gray', linewidth=1)
    ...
    ax.text(8, 4, '$\\tau_{switch} = 10^{-6}$ (ULP-based)', ha='center', va='center', fontsize=10,
    ...
    # Add overall title
    plt.suptitle('ZeroProofML: Architecture and Computational Flow', 
                fontsize=16, fontweight='bold', y=0.95)
    
    # Add subtitle
    ax.text(7.5, 10.8, 'Singularity-Resilient Learning with Transreal Arithmetic', 
...
def create_alternative_flow_diagram():
    ...
    for x, y, text in processes:
        ...
        box = FancyBboxPatch((x-1, y-0.3), 2, 0.6, boxstyle="round,pad=0.05",
    ...
    diamond = plt.Polygon(diamond_points, facecolor=colors['decision'], 
    ...
    guard_box = FancyBboxPatch((1.5, 4.8), 3, 1.4, boxstyle="round,pad=0.1",
                              facecolor=colors['guard'], edgecolor='#2196F3', linewidth=2)
    ax.add_patch(guard_box)
    ...
    real_box = FancyBboxPatch((9.5, 4.8), 3, 1.4, boxstyle="round,pad=0.1",
                             facecolor=colors['real'], edgecolor='#F44336', linewidth=2)
    ax.add_patch(real_box)
    ...
    ax.text(11, 4.9, 'Tag: INF/NULL', ha='center', va='center', fontsize=9, color='red', weight='bold')
    
    # Coverage controller (side panel)
    controller_box = FancyBboxPatch((9, 7), 4, 1.5, boxstyle="round,pad=0.1",
                                   facecolor=colors['control'], edgecolor='#8BC34A', linewidth=2)
    ax.add_patch(controller_box)
    ax.text(11, 8, '\\textbf{Coverage Controller}', ha='center', va='center', fontsize=11, weight='bold')
    ...
    # Main flow
    ax.annotate('', xy=(6, 6.2), xytext=(6, 7.2), 
                arrowprops=dict(arrowstyle='->', lw=3, color='black'))
    ax.annotate('', xy=(6, 5.8), xytext=(6, 6.2),
                arrowprops=dict(arrowstyle='->', lw=3, color='black'))
    ax.annotate('', xy=(6, 4.8), xytext=(6, 4.9),
                arrowprops=dict(arrowstyle='->', lw=3, color='black'))
    ax.annotate('', xy=(6, 1.8), xytext=(6, 4.2),
                arrowprops=dict(arrowstyle='->', lw=3, color='black'))
    
    # Decision branches
    ax.annotate('', xy=(4.5, 5.5), xytext=(5.4, 5.5),
                arrowprops=dict(arrowstyle='->', lw=2.5, color='blue'))
    ax.text(4.8, 5.8, 'YES', fontsize=10, color='blue', weight='bold')
    
    ax.annotate('', xy=(7.5, 5.5), xytext=(6.6, 5.5),
                arrowprops=dict(arrowstyle='->', lw=2.5, color='red'))
    ax.text(7.2, 5.8, 'NO', fontsize=10, color='red', weight='bold')
    
    # Convergence arrows
    ax.annotate('', xy=(5.5, 4), xytext=(3, 4.8),
                arrowprops=dict(arrowstyle='->', lw=2, color='blue', 
                               connectionstyle="arc3,rad=0.3"))
    ax.annotate('', xy=(6.5, 4), xytext=(11, 4.8),
                arrowprops=dict(arrowstyle='->', lw=2, color='red',
                               connectionstyle="arc3,rad=-0.3"))
    
    # Coverage feedback
    ax.annotate('', xy=(9.5, 7.5), xytext=(9, 8.5),
