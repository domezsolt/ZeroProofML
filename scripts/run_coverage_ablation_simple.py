...
def simulate_coverage_control_impact():
    ...
    ax1.plot(epochs_arr, losses_with, 'o-', label='With Coverage', color='green', linewidth=2, markersize=4)
    ax1.plot(epochs_arr, losses_without, 's-', label='Without Coverage', color='red', linewidth=2, markersize=4)
    ...
    ax2.plot(epochs_arr, np.array(coverage_with) * 100, 'o-',
            label='With Controller', color='green', linewidth=2, markersize=4)
    ax2.plot(epochs_arr, np.array(coverage_without) * 100, 's-',
            label='Without Controller', color='red', linewidth=2, markersize=4)
    ...
    ax2.fill_between(epochs_arr, np.array(coverage_without) * 100, 
                     alpha=0.2, color='red', label='Coverage collapse')
    ...
    for i, (w, wo) in enumerate(zip(with_mse, without_mse)):
        ...
        ax3.text(i + width/2, wo + 0.0002, f'+{degradation:.0f}%', 
    ...
    results = {
        'with_coverage': {
            ...
            'loss_history': [float(l) for l in losses_with]
        },
        'without_coverage': {
            ...
            'loss_history': [float(l) for l in losses_without]
        },
        'improvements': {
            'B0_degradation_pct': (buckets_without['B0']['mse'] - buckets_with['B0']['mse']) / buckets_with['B0']['mse'] * 100,
            'B1_degradation_pct': (buckets_without['B1']['mse'] - buckets_with['B1']['mse']) / buckets_with['B1']['mse'] * 100,
