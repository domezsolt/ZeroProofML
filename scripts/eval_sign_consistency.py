...
def main():
    ap = argparse.ArgumentParser(description='Evaluate sign consistency across θ2=0 for E1 quick runs')
    ap.add_argument('--dataset', default='data/rr_ik_dataset.json', help='Path to dataset JSON')
    ap.add_argument('--runs_glob', default='results/robotics/quick_s*', help='Glob for quick run directories')
    ap.add_argument('--outjson', default=None, help='Optional path to write aggregated JSON')
    ap.add_argument('--outfig', default='results/robotics/figures/e1_sign_consistency.png', help='Output figure path')
    ...
    per_method: Dict[str, List[float]] = {'MLP': [], 'Rational+ε': [], 'ZeroProofML-Basic': [], 'ZeroProofML-Full': []}
    ...
    for rd in run_dirs:
        ...
        for method, preds in preds_map.items():
            ...
            score = compute_sign_consistency_rate(test_inputs[:n], preds_f, n_paths=args.n_paths,
                                                  th1_tol=args.th1_tol, th2_window=args.th2_window)
            per_method[method].append(score)

    # Aggregate and print
    summary = {'methods': {}, 'params': {
