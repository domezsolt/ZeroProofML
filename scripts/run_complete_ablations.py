...
def create_ablation_visualizations(thresh_results, policy_results, component_analysis):
    ...
    line1 = ax2.plot(
        x, switching_freq, "o-", color="blue", linewidth=2, label="Switching Frequency"
    )
    ...
    for i in range(len(components)):
        for j in range(len(aspects)):
            ...
            ax5.text(
                ...
                ha="center",
                va="center",
                color="white" if score > 1.5 else "black",
                fontsize=9,
            )
    ...
    plt.suptitle(
        "Comprehensive Ablation Studies: ZeroProofML Components", fontsize=14, fontweight="bold"
    )
    plt.tight_layout()
