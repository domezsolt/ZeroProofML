...
def create_professional_flow():
    ...
    # 1. Input Box
    input_box = FancyBboxPatch((1, 3), 2.5, 1.2, 
    ...
    tr_box = FancyBboxPatch((5, 2.5), 3.5, 2, 
                           boxstyle="round,pad=0.08",
    ...
    guard_box = FancyBboxPatch((11.5, 5), 3, 1.2,
                              boxstyle="round,pad=0.08",
    ...
    real_box = FancyBboxPatch((11.5, 1.5), 3, 1.2,
                             boxstyle="round,pad=0.08",
    ...
    # 6. Output Box
    output_box = FancyBboxPatch((16, 3), 2.5, 1.2,
                               boxstyle="round,pad=0.08",
    ...
    # Input → TR-Rational
    ax.add_patch(FancyArrowPatch((3.5, 3.6), (5, 3.6), 
                                connectionstyle="arc3", **arrow_kwargs))
    
    # TR-Rational → Decision
    ax.add_patch(FancyArrowPatch((8.5, 3.5), (9.3, 3.5),
                                connectionstyle="arc3", **arrow_kwargs))
    
    # Decision → Guard (YES)
    ax.add_patch(FancyArrowPatch((10.3, 3.9), (11.5, 5.3),
                                connectionstyle="arc3,rad=.3",
                                arrowstyle=arrow_style, color="#388E3C", lw=2))
    add_text(10.5, 4.5, 'YES', fontsize=10, color='#388E3C', fontweight='bold',
            bbox=dict(boxstyle="round,pad=0.2", facecolor='white', edgecolor='#388E3C'))
    
    # Decision → Real (NO)
    ax.add_patch(FancyArrowPatch((10.3, 3.1), (11.5, 2.2),
                                connectionstyle="arc3,rad=-.3",
                                arrowstyle=arrow_style, color="#D32F2F", lw=2))
    add_text(10.5, 2.5, 'NO', fontsize=10, color='#D32F2F', fontweight='bold',
            bbox=dict(boxstyle="round,pad=0.2", facecolor='white', edgecolor='#D32F2F'))
    
    # Guard → Output
    ax.add_patch(FancyArrowPatch((14.5, 5.5), (16.2, 3.8),
                                connectionstyle="arc3,rad=-.3",
                                arrowstyle=arrow_style, color="#FF8F00", lw=1.8))
    
    # Real → Output
    ax.add_patch(FancyArrowPatch((14.5, 2), (16.2, 3.2),
                                connectionstyle="arc3,rad=.3",
                                arrowstyle=arrow_style, color="#D32F2F", lw=1.8))
    
    # Add key components labels
    add_text(6.75, 1.5, 'Computational Core', fontsize=11, fontweight='bold', 
            color=colors['accent'], style='italic')
    
    add_text(13, 0.5, 'Dual-Path Architecture', fontsize=11, fontweight='bold',
            color=colors['accent'], style='italic')
    
    # Add threshold notation
    add_text(10, 2.3, 'τ_switch = 10⁻⁶', fontsize=9, style='italic',
    ...
    # Title and subtitle
    add_text(10, 6.5, 'ZeroProofML: Transreal Computation Flow', 
            fontsize=16, fontweight='bold')
    add_text(10, 6.0, 'Singularities become well-defined computational states',
    ...
    plt.savefig(output_dir / 'figure6_final_schematic.png', dpi=150, bbox_inches='tight', 
                facecolor='white', edgecolor='none')
    plt.savefig(output_dir / 'figure6_final_schematic.pdf', dpi=300, bbox_inches='tight',
                facecolor='white', edgecolor='none')
