...
def build_table(
    rows: List[Dict[str, str]], methods: List[str], buckets: List[str], caption: str, label: str
) -> str:
    # Header row
...
def main():
    ...
    ap.add_argument(
        "--methods", nargs="*", default=None, help="List of method names to include (in order)"
    )
    ap.add_argument(
        "--buckets", nargs="*", default=None, help="Buckets to include (default: B0 B1 B2 B3 B4)"
    )
    ap.add_argument(
        "--caption",
        default="Per-bucket MSE (mean) with counts (n). Lower is better.",
        help="LaTeX caption",
    )
    ap.add_argument("--label", default="tab:buckets_mse", help="LaTeX label")
    ap.add_argument(
        "--out", default="results/robotics/aggregated/buckets_table.tex", help="Output .tex file"
    )
    args = ap.parse_args()
