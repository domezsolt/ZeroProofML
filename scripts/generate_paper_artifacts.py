...
def collect_bins(root: str, methods: List[str]) -> Tuple[List[str], Dict[str, Dict[str, Tuple[float, float, int]]]]:
    pattern = os.path.join(root, 'seed_*', 'comprehensive_comparison.json')
    files = sorted(glob.glob(pattern))
...
def write_bins_table(path: str, keys: List[str], stats: Dict[str, Dict[str, Tuple[float, float, int]]], methods: List[str]) -> None:
    def _fmt_method(name: str) -> str:
        return name.replace('ε', '$\\varepsilon$')
...
def write_e3r_table(path: str, e3r_json: str) -> None:
    ...
    if 'sign_consistency_theta2' in pole:
        lines.append(f'    Sign consistency ($\\theta_2$) & {pole.get("sign_consistency_theta2"):.3f} & fraction of anchors \\\\')
    if 'sign_consistency_theta3' in pole:
        lines.append(f'    Sign consistency ($\\theta_3$) & {pole.get("sign_consistency_theta3"):.3f} & fraction of anchors \\\\')
    if 'residual_consistency' in pole:
        lines.append(f'    Residual consistency & {pole.get("residual_consistency"):.6f} & FK error \\\\')
    lines.append('    \\bottomrule')
    lines.append('  \\end{tabular}')
...
def write_ik6r_table(path: str, ik6r_csv: str) -> None:
    ...
    for r in rows:
        ...
        if name == 'overall_mse':
            lines.append(f'    Overall MSE & {float(mu):.6f} & {float(sd):.6f} & {int(float(n))} & 6R TR \\\\')
        # Show a couple of bins
        if name in ('(0e+00,1e-05]', '(1e-05,1e-04]', '(1e-04,1e-03]'):
            lines.append(f'    {name} & {float(mu):.6f} & {float(sd):.6f} & {int(float(n))} & 6R bins \\\\')
    lines.append('    \\bottomrule')
    lines.append('  \\end{tabular}')
...
def main() -> None:
    ...
    ap.add_argument('--root', required=True, help='Output root (e.g., results/robotics/paper_suite)')
    ap.add_argument('--methods', nargs='*', default=['ZeroProofML-Full', 'Rational+ε', 'Smooth', 'Learnable-ε', 'EpsEnsemble', 'MLP'])
    ap.add_argument('--latex-dir', default='latex', help='Subdirectory under root to write LaTeX files')
    ...
    if args.with_rollout:
        write_rollout_table(os.path.join(outdir, 'rollout_table.tex'), os.path.join(root, 'rollout_summary.json'))

    # 3R
    if args.with_3r:
        write_e3r_table(os.path.join(outdir, 'e3r_table.tex'), os.path.join(root, 'e3r', 'e3r_results.json'))

    # 6R
    if args.with_6r:
        write_ik6r_table(os.path.join(outdir, 'ik6r_table.tex'), os.path.join(root, 'aggregated', 'ik6r_summary.csv'))
