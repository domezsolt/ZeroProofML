    ...
    for epoch in range(epochs):
        ...
        if epoch % 2 == 0:
            ...
            with torch.no_grad():
                ...
                print(
                    f"Epoch {epoch+1:3d}: Train Loss={loss.item():.4f}, Test MSE={test_mse:.4f}, "
...
def main():
    ...
    # Overall MSE
    print(
        f"{'Overall Test MSE':<30} {results_with['final_test_mse']:<20.6f} {results_without['final_test_mse']:<20.6f}"
    )
    ...
    for bucket in ["B0", "B1", "B2"]:
        ...
        if with_mse > 0 and without_mse > 0:
            ...
            print(
                f"{bucket + ' MSE':<30} {with_mse:<20.6f} {without_mse:<20.6f} ({improvement:+.1f}%)"
            )
        else:
    ...
    # Training time
    print(
        f"{'Training Time (s)':<30} {results_with['train_time']:<20.2f} {results_without['train_time']:<20.2f}"
    )
    ...
    ablation_results = {
        ...
        "analysis": {
            "b0_improvement": (
                results_without["bucket_mse"].get("B0", {}).get("mse", 0)
                - results_with["bucket_mse"].get("B0", {}).get("mse", 0)
            ),
            "b1_improvement": (
                results_without["bucket_mse"].get("B1", {}).get("mse", 0)
    ...
    try:
        ...
        ax1.plot(
            results_without["train_losses"], label="Without Coverage", color="red", linewidth=2
        )
        ...
        ax2.plot(
            epochs,
            np.array(results_with["coverage_history"]) * 100,
            marker="o",
        ...
        ax2.plot(
            epochs,
            np.array(results_without["coverage_history"]) * 100,
            marker="s",
        ...
        bars1 = ax3.bar(
            x - width / 2, with_mse, width, label="With Coverage", color="green", alpha=0.7
        )
        bars2 = ax3.bar(
