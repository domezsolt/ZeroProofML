...
def write_latex(
    path: str, agg: Dict[str, Dict[str, Tuple[float, float, int]]], methods: List[str]
) -> None:
    os.makedirs(os.path.dirname(path), exist_ok=True)
    # Select columns
    ...
    lines.append(
        "  \\caption{Across-seed mean$\\pm$std (3 seeds) for overall MSE and near-pole buckets on the exact dataset.}"
    )
