...
def create_improved_coverage_figure():
    ...
    ax1.plot(epochs_arr, losses_with, 'o-', label='With Coverage', color='green', linewidth=2.5, markersize=5)
    ax1.plot(epochs_arr, losses_without, 's-', label='Without Coverage', color='red', linewidth=2.5, markersize=5)
    ...
    ax2 = axes[1]
    ax2.plot(epochs_arr, np.array(coverage_with) * 100, 'o-',
            label='With Controller', color='green', linewidth=2.5, markersize=5)
    ax2.plot(epochs_arr, np.array(coverage_without) * 100, 's-',
            label='Without Controller', color='red', linewidth=2.5, markersize=5)
    ...
    ax2.fill_between(epochs_arr, np.array(coverage_without) * 100, 
    ...
    bars1 = ax3.bar(x - width/2, with_mse, width, label='With Coverage', color='green', alpha=0.8, edgecolor='darkgreen')
    bars2 = ax3.bar(x + width/2, without_mse, width, label='Without Coverage', color='red', alpha=0.8, edgecolor='darkred')
    ...
    for i, (w, wo) in enumerate(zip(with_mse, without_mse)):
        ...
        y_pos = max(w, wo) * 1.1
        ax3.text(i, y_pos, f'+{degradation:.0f}%', 
    ...
    plt.suptitle('Coverage Control Ablation: Preventing Near-Singularity Avoidance', 
...
def create_zeroproofml_schematic():
    ...
    # Input layer
    input_box = plt.Rectangle((1, 7), 2, 1, facecolor=colors['input'], 
                             edgecolor='black', linewidth=1.5)
    ax.add_patch(input_box)
    ax.text(2, 7.5, 'Input\n$x \\in \\mathbb{R}^n$', ha='center', va='center', fontsize=10, weight='bold')
    
    # TR-Rational Layer
    tr_box = plt.Rectangle((5, 6.5), 3, 2, facecolor=colors['tr_layer'], 
                          edgecolor='darkgreen', linewidth=2)
    ax.add_patch(tr_box)
    ax.text(6.5, 7.5, 'TR-Rational Layer\n$P(x)/Q(x)$', ha='center', va='center', fontsize=11, weight='bold')
    
    # Condition checking
    condition_diamond = plt.Polygon([(6.5, 5.5), (7.5, 5), (6.5, 4.5), (5.5, 5)], 
    ...
    ax.arrow(10, 6, -1.2, 1, head_width=0.1, head_length=0.08, fc='brown', ec='brown', linestyle='--')
    ax.text(9.2, 6.8, 'Feedback', fontsize=8, color='brown', style='italic')
    
    # Output
    output_box = plt.Rectangle((5, 0), 3, 1, facecolor=colors['output'], 
                              edgecolor='purple', linewidth=2)
    ax.add_patch(output_box)
    ax.text(6.5, 0.5, 'Output: $(y, tag) \\in \\mathbb{T}$', ha='center', va='center', fontsize=11, weight='bold')
    ...
    plt.title('ZeroProofML Architecture and Computational Flow', fontsize=14, fontweight='bold', pad=20)
    plt.tight_layout()
...
def fix_coverage_ablation_figure():
    ...
    bars2 = ax3.bar(x + width/2, without_mse, width, label='Without Coverage', color='red', alpha=0.8)
