...
def create_perfect_schematic():
    ...
    ax.text(input_x, input_y, "Input", ha="center", va="center", fontsize=12, fontweight="bold")
    ...
    ax.text(
        guard_x, guard_y, "Guard Mode", ha="center", va="center", fontsize=11, fontweight="bold"
    )
    ...
    ax.text(real_x, real_y, "Real Mode", ha="center", va="center", fontsize=11, fontweight="bold")
    ...
    ax.text(output_x, output_y, "Output", ha="center", va="center", fontsize=12, fontweight="bold")
    ...
    # Input → TR-Rational
    ax.annotate("", xy=(tr_x - 1.5, tr_y), xytext=(input_x + 1, input_y), arrowprops=arrow_style)

    # TR-Rational → Decision
    ax.annotate(
        "", xy=(decision_x - 0.5, decision_y), xytext=(tr_x + 1.5, tr_y), arrowprops=arrow_style
    )

    # Decision → Guard (YES)
    ax.annotate(
        "",
        xy=(guard_x - 1.25, guard_y - 0.2),
        xytext=(decision_x + 0.3, decision_y + 0.3),
        arrowprops=dict(arrowstyle="->", lw=2, color="#388E3C", connectionstyle="arc3,rad=0.2"),
    )
    ax.text(
        decision_x + 0.8,
        decision_y + 0.8,
        ...
        fontweight="bold",
        ha="center",
        bbox=dict(boxstyle="round,pad=0.15", facecolor="white", edgecolor="#388E3C", linewidth=1),
    )

    # Decision → Real (NO)
    ax.annotate(
        "",
        xy=(real_x - 1.25, real_y + 0.2),
        xytext=(decision_x + 0.3, decision_y - 0.3),
        arrowprops=dict(arrowstyle="->", lw=2, color="#D32F2F", connectionstyle="arc3,rad=-0.2"),
    )
    ax.text(
        decision_x + 0.8,
        decision_y - 0.8,
        ...
        fontweight="bold",
        ha="center",
        bbox=dict(boxstyle="round,pad=0.15", facecolor="white", edgecolor="#D32F2F", linewidth=1),
    )

    # Guard → Output
    ax.annotate(
        "",
        xy=(output_x - 1, output_y + 0.2),
        xytext=(guard_x + 1.25, guard_y - 0.2),
        arrowprops=dict(arrowstyle="->", lw=2, color="#FF8F00", connectionstyle="arc3,rad=-0.2"),
    )
    ...
    ax.text(
        ...
        fontsize=9,
        style="italic",
        bbox=dict(boxstyle="round,pad=0.25", facecolor="white", edgecolor="orange", linewidth=1),
    )
    ...
    plt.savefig(
        output_dir / "figure6_perfect_schematic.png",
        dpi=150,
        bbox_inches="tight",
        facecolor="white",
        edgecolor="none",
    ...
    plt.savefig(
        output_dir / "figure6_perfect_schematic.pdf",
        dpi=300,
        bbox_inches="tight",
        facecolor="white",
        edgecolor="none",
...
def create_ultra_clean_version():
    ...
    # Minimalist colors
    colors = {"process": "#F5F5F5", "guard": "#E8F5E9", "real": "#FFEBEE", "decision": "#FFF3E0"}
    ...
    circle = Circle((x, y), 0.25, facecolor=colors["decision"], edgecolor="#FF9800", linewidth=1.5)
    ax.add_patch(circle)
    ...
    for i, arrow_info in enumerate(arrows):
        ...
        if label:
            ...
            ax.text(
                mid_x, mid_y + 0.1, label, fontsize=8, color=color, fontweight="bold", ha="center"
            )

    # Title (with space above)
    ax.text(
        7, 5, "ZeroProofML Architecture", ha="center", va="center", fontsize=14, fontweight="bold"
    )
