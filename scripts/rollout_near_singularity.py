...
def prepare_data(ds: Dict[str, Any], max_train: int = 2000) -> Tuple[Tuple[List, List], Tuple[List, List]]:
    samples = ds['samples']
...
def train_models_quick(train, val) -> Dict[str, Any]:
    ...
    # Rational+ε quick
    rat_cfg = RationalEpsConfig(epochs=3, epsilon_values=[1e-4, 1e-3], learning_rate=1e-2, batch_size=32)
    ...
    rat_tr = RationalEpsTrainer(rat, Optimizer(rat.parameters(), learning_rate=rat_cfg.learning_rate))
    rat_tr.train(*train, *val, verbose=False)
    models['Rational+ε'] = rat

    # TR Basic
    GradientModeConfig.set_mode(GradientMode.MASK_REAL)
    tr_basic = TRMultiInputRational(input_dim=4, n_outputs=2, d_p=3, d_q=2, hidden_dims=[8], shared_Q=True)
    # tiny loop
    ...
    # TR Full (Hybrid)
    tr_full = TRMultiInputRational(input_dim=4, n_outputs=2, d_p=3, d_q=2, hidden_dims=[8], shared_Q=True)
...
def predict_step(method: str, model, theta1: float, theta2: float, dx: float, dy: float) -> Tuple[float, float, bool]:
    non_real = False
    # Build TRNodes
    if method == 'Rational+ε':
        ...
        tr_inputs = [TRNode.constant(real(theta1)), TRNode.constant(real(theta2)),
                     TRNode.constant(real(dx)), TRNode.constant(real(dy))]
...
def rollout(models: Dict[str, Any], n_traj: int = 4, n_steps: int = 50,
            start_theta2: float = 0.03, step_size: float = 0.01,
            phi_list: List[float] = (0.0, 90.0)) -> Dict[str, Any]:
...
def main():
    ...
    summary = rollout(models, n_traj=args.n_traj, n_steps=args.n_steps,
                      start_theta2=args.start_theta2, step_size=args.step_size,
                      phi_list=[0.0, 90.0])
    with open(args.out, 'w') as fh:
        json.dump(summary, fh, indent=2)
    print('Rollout summary:')
    for k, v in summary.items():
        print(f"{k:18s}  mean_err={v['mean_tracking_error']:.4f}  max_dtheta={v['max_joint_step']:.4f}  failure%={v['failure_rate']:.2f}")
