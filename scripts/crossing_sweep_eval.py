...
def paired_consistency_with_direction(test_inputs, preds, n_paths, th1_tol, th2_window,
                                      k_pairs, min_dtheta2,
                                      phi_deg, phi_tol_deg) -> Tuple[float, int]:
    ...
    def ang_diff(a, b):
        ...
        return np.abs(d)
    for a in anchors:
        mask = (np.abs(th1 - a) <= th1_tol) & (np.abs(th2) <= th2_window) & (ang_diff(ang, phi_deg) <= phi_tol_deg)
        idx = np.where(mask)[0]
        if idx.size < 2:
            continue
        neg = idx[th2[idx] < 0.0]
...
def main():
    ap = argparse.ArgumentParser(description='Direction-fixed crossing sweep consistency (no retraining)')
    ...
    ap.add_argument('--phi_deg', type=float, default=0.0, help='Desired displacement direction angle (deg)')
    ...
    per_method_scores: Dict[str, List[float]] = {'MLP': [], 'Rational+ε': [], 'ZeroProofML-Basic': [], 'ZeroProofML-Full': []}
    ...
    for rd in run_dirs:
        ...
        for method, preds in preds_map.items():
            ...
            score, count = paired_consistency_with_direction(
                test_inputs, preds,
                args.n_paths, args.th1_tol, args.th2_window,
                args.k_pairs, args.min_dtheta2,
                args.phi_deg, args.phi_tol_deg,
            )
    ...
    for m, xs in per_method_scores.items():
        ...
        summary['methods'][m] = {
            'mean': mu, 'std': sd, 'n': len(xs), 'values': per_method_scores[m],
        ...
        print(f"  {m:18s}: {mu*100:.2f}% ± {sd*100:.2f}% (pairs~{summary['methods'][m]['pairs_mean']:.1f})")
