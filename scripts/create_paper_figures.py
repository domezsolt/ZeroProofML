...
def create_figure1_nearpole_bars():
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(['ZeroProofML', 'ε-Ensemble', 'Smooth', 'Learnable-ε', 'MLP'], rotation=15, ha='right')
    ...
    for i in range(1, len(methods)):
        if methods[i] == 'EpsEnsemble':
            ...
            ax.text(0, 0.0045, f'29.7% ↓', ha='center', fontsize=8, color='darkgreen', weight='bold')
            ax.text(0, 0.0028, f'46.6% ↓', ha='center', fontsize=8, color='darkgreen', weight='bold')
            break
    
    plt.tight_layout()
    plt.savefig('/home/zsemed/ZeroProofML/results/robotics/paper_suite/figures/figure1_nearpole_bars.pdf', dpi=300, bbox_inches='tight')
    plt.savefig('/home/zsemed/ZeroProofML/results/robotics/paper_suite/figures/figure1_nearpole_bars.png', dpi=150, bbox_inches='tight')
...
def create_figure2_conceptual():
    ...
    ax1.annotate('', xy=(0.5, 2), xytext=(0.5, 1.6),
    ...
    ax2.plot(x_left, np.clip(y_left, -3, 3), 'g-', linewidth=2, alpha=0.8, label='TR: 1/x (REAL tag)')
    ...
    plt.suptitle('Division by Zero: Traditional vs Transreal Handling', fontsize=12, fontweight='bold')
    plt.tight_layout()
    
    plt.savefig('/home/zsemed/ZeroProofML/results/robotics/paper_suite/figures/figure2_conceptual.pdf', dpi=300, bbox_inches='tight')
    plt.savefig('/home/zsemed/ZeroProofML/results/robotics/paper_suite/figures/figure2_conceptual.png', dpi=150, bbox_inches='tight')
...
def create_figure3_ablation():
    ...
    data_file = "/home/zsemed/ZeroProofML/results/robotics/paper_suite/seed_1/comprehensive_comparison.json"
    ...
    bucket_keys = ['(0e+00,1e-05]', '(1e-05,1e-04]', '(1e-04,1e-03]', '(1e-03,1e-02]', '(1e-02,inf]']
    ...
    basic_vals = [
        tr_basic['timing']['total_seconds'] / 60 if 'timing' in tr_basic else 3.0,  # Convert to minutes
        tr_basic['config']['n_params'] / 10 if 'n_params' in tr_basic['config'] else 7.0,  # Scale for visibility
    ...
    imp_rollout = (rollout_data['Mask-REAL'] - rollout_data['Hybrid']) / rollout_data['Mask-REAL'] * 100
    ax3.text(2, full_display[2] + 0.5, f'{imp_rollout:.1f}% ↓', ha='center', fontsize=8, color='darkgreen', weight='bold')
    
    plt.suptitle('Ablation Study: Impact of Hybrid Switching', fontsize=12, fontweight='bold')
    plt.tight_layout()
    
    plt.savefig('/home/zsemed/ZeroProofML/results/robotics/paper_suite/figures/figure3_ablation.pdf', dpi=300, bbox_inches='tight')
    plt.savefig('/home/zsemed/ZeroProofML/results/robotics/paper_suite/figures/figure3_ablation.png', dpi=150, bbox_inches='tight')
