...
def compute_near_pole_indices(samples: List[Dict[str, Any]], n_train: int, thresh: float) -> List[int]:
    idxs = []
...
def main():
    ap = argparse.ArgumentParser(description='Plot ε-sensitivity at exact det(J)=0 and report zero-norm rates')
    ap.add_argument('--dataset', required=True, help='Path to dataset JSON (generated by rr_ik_dataset.py)')
    ap.add_argument('--results', nargs='+', required=True, help='List of trainer results JSON files to include')
    ap.add_argument('--labels', nargs='*', default=None, help='Optional labels for each results file')
    ap.add_argument('--out', default='results/robotics/figures/pole_exact_scatter.png', help='Output figure path')
    ap.add_argument('--detj_thresh', type=float, default=0.0, help='Select samples with |detJ| ≤ threshold (default: 0 for exact poles)')
    args = ap.parse_args()
    ...
    if not near_idxs:
        if args.detj_thresh <= 0.0:
            print('No exact det(J)=0 samples found in test split; consider using --detj_thresh 1e-5 for B0.')
        else:
    ...
    if x:
        ...
        ylabel = f"||Δθ|| at |det J| ≤ {args.detj_thresh:g}" if args.detj_thresh > 0 else "||Δθ|| at det(J)=0"
        plt.ylabel(ylabel)
