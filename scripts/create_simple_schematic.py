...
def create_clean_schematic():
    ...
    # Input
    input_rect = FancyBboxPatch((0.5, 2), 2, 1, boxstyle="round,pad=0.1",
                               facecolor=colors['input'], edgecolor='#1976D2', linewidth=2)
    ax.add_patch(input_rect)
    ax.text(1.5, 2.5, 'Input\\n$x \\in \\mathbb{R}^n$', ha='center', va='center', fontsize=11, weight='bold')
    
    # TR-Rational Layer
    tr_rect = FancyBboxPatch((4, 1.5), 3, 2, boxstyle="round,pad=0.1",
                            facecolor=colors['layer'], edgecolor='#388E3C', linewidth=2)
    ...
    # Guard path
    guard_rect = FancyBboxPatch((10, 3.5), 2.5, 0.8, boxstyle="round,pad=0.05",
                               facecolor=colors['guard'], edgecolor='#FF9800', linewidth=1.5)
    ...
    # Real path
    real_rect = FancyBboxPatch((10, 1.2), 2.5, 0.8, boxstyle="round,pad=0.05",
                              facecolor=colors['real'], edgecolor='#F44336', linewidth=1.5)
    ax.add_patch(real_rect)
    ax.text(11.25, 1.6, 'Real: $\\pm\\infty/\\Phi$', ha='center', va='center', fontsize=10, weight='bold')
    ax.text(11.25, 1.3, '(INF/NULL)', ha='center', va='center', fontsize=9, color='red')
    
    # Output
    output_rect = FancyBboxPatch((14, 2), 2, 1, boxstyle="round,pad=0.1",
                                facecolor=colors['output'], edgecolor='#9C27B0', linewidth=2)
    ...
    # Paths to output
    ax.arrow(12.5, 3.9, 1.2, -1.2, head_width=0.08, head_length=0.08, fc='orange', ec='orange', lw=1.5)
    ax.arrow(12.5, 1.6, 1.2, 0.7, head_width=0.08, head_length=0.08, fc='red', ec='red', lw=1.5)
    
    # Add condition text
    ax.text(8.5, 1.5, '$|Q| > \\tau_{switch}$', ha='center', va='center', fontsize=10,
            bbox=dict(boxstyle="round,pad=0.2", facecolor='white', edgecolor='orange'))
    
    # Add key insight
    ax.text(8.5, 0.5, '\\textbf{Key:} Singularities become well-defined computational states', 
    ...
    # Title
    ax.text(8.5, 4.8, '\\textbf{ZeroProofML: Transreal Computation Flow}', 
