...
def _fit_text_inside(ax, txt, w_data, h_data,
                     pad_px=14, min_fs=8.0, max_fs=11.0):
...
def add_box(ax, xy, w, h, text, max_fontsize=11.0):
    ...
    rect = FancyBboxPatch(
        ...
        linewidth=1.3, facecolor=BOX_FACE, edgecolor=BOX_EDGE, zorder=1.0
    )
    ax.add_patch(rect)
    t = ax.text(
        xy[0] + w/2, xy[1] + h/2, text,
        ha="center", va="center", fontsize=max_fontsize,
        color=TEXT_COLOR, linespacing=1.2, wrap=True,
...
def fit_all_text_after_limits(ax, boxes):
    for b in boxes:
        _fit_text_inside(ax, b["text"], b["w"], b["h"],
                         pad_px=14, min_fs=8.2, max_fs=b["max_fontsize"])
...
def arrow(ax, p0, p1, rad=0.0, mscale=9, lw=1.15, z=2.1):
    ...
    arr = FancyArrowPatch(
        ...
        mutation_scale=mscale, lw=lw,
        ec=ARROW_COLOR, fc="none",
...
def main():
    ...
    # boxes
    inputs = add_box(ax, (0.5, core_y), core_w, core_h,
                     r"Inputs" "\n" r"$[\theta_1,\theta_2,\Delta x,\Delta y]$")
    ops = add_box(ax, (inputs["right"] + gap, core_y), core_w, core_h,
                  r"Totalized TR ops" "\n" r"(no undefined $+,\times,\div$)")
    tags = add_box(ax, (ops["right"] + gap, core_y), core_w, core_h,
                   r"Tagged values" "\n" r"REAL / $\pm\infty$ / $\Phi$")
    mask = add_box(ax, (tags["right"] + gap, core_y + core_h + 0.50),
                   branch_w, branch_h,
                   r"Mask-REAL gradients" "\n" r"REAL path $\Rightarrow$ classical derivatives")
    sat  = add_box(ax, (tags["right"] + gap, core_y - branch_h - 0.50),
                   branch_w, branch_h,
                   r"Saturating gradients" "\n" r"Bounded updates when $|Q(x)|\ll 1$")
    outs = add_box(ax, (mask["right"] + gap, core_y), core_w, core_h,
    ...
    ax.text((tags["mid_x"] + outs["mid_x"]) / 2, top_y + 1.02,
            "Hybrid scheduler selects REAL vs. saturating branch",
            ha="center", va="center", fontsize=11.0, color=ARROW_COLOR,
            bbox=dict(facecolor="white", alpha=0.96, pad=2.2, edgecolor="none"),
            zorder=3.0, clip_on=False)
    ...
    arrow(ax,
          (inputs["right"] + margin, arrow_y),
          (ops["left"]   - margin, arrow_y),
          rad=0.0)
    arrow(ax,
          (ops["right"]  + margin, arrow_y),
          (tags["left"]  - margin, arrow_y),
          rad=0.0)

    # split from tags -> (mask, sat) (same size both)
    arrow(ax,
          (tags["right"] + margin, arrow_y),
          (mask["left"]  - margin, mask["mid_y"]),
          rad=0.10)
    arrow(ax,
          (tags["right"] + margin, arrow_y),
          (sat["left"]   - margin, sat["mid_y"]),
          rad=-0.10)
    ...
    arrow(ax,
          (mask["right"] - margin, mask["mid_y"]),
          (outs["left"]  + margin, join_up),
          rad=-0.07)
    arrow(ax,
          (sat["right"]  - margin, sat["mid_y"]),
          (outs["left"]  + margin, join_dn),
          rad=0.07)
