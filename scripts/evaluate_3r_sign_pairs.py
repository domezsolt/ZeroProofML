...
def main():
    ap = argparse.ArgumentParser(
        description="Evaluate 3R paired sign consistency under direction window"
    )
    ap.add_argument("--results", required=True, help="Path to comprehensive_comparison_3r.json")
    ap.add_argument(
        "--phi", type=float, default=60.0, help="Direction angle in degrees (atan2(dy,dx))"
    )
    ap.add_argument(
        "--phi_tol", type=float, default=35.0, help="Direction angle tolerance in degrees"
    )
    ap.add_argument("--k", type=int, default=4, help="Number of pairs")
    ap.add_argument(
        "--th_window", type=float, default=0.35, help="Window around zero for |theta_j|"
    )
    ...
    for display, key in [
        ...
        tinp = test_inputs[:m]
        pp = preds[:m]
        th2 = compute_paired_sign_consistency_3r(
            tinp,
            pp,
            joint="theta2",
            phi_deg=args.phi,
            phi_tol_deg=args.phi_tol,
            th_window=args.th_window,
            k=args.k,
            min_mag=args.min_mag,
        )
        th3 = compute_paired_sign_consistency_3r(
            tinp,
            pp,
            joint="theta3",
            phi_deg=args.phi,
    ...
    print(
        f"phi={args.phi}±{args.phi_tol} deg, k={args.k}, |theta_j|≤{args.th_window}, min|dtheta_j|={args.min_mag}"
    )
    for name, stats in summary["models"].items():
        t2 = stats["theta2"]
        t3 = stats["theta3"]
        print(
            f"- {name:12s}  theta2: {t2['rate']*100:5.2f}% (pairs={t2['pairs']}),  theta3: {t3['rate']*100:5.2f}% (pairs={t3['pairs']})"
        )
